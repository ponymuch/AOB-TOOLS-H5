...
HeroData.DefaultHero.MaxHealth = 1000
HeroData.DefaultHero.HardModeStartingResources = {
			GiftPoints = 99999999,
			MetaPoints = 99999999,
			Gems = 99999999,
			LockKeys = 99999999,
			SuperLockKeys = 99999999,
			SuperGems = 99999999,
			SuperGiftPoints = 99999999,
		}
HeroData.DefaultHero.EnemyMoneyDropBaseValue = 500.0
HeroData.DefaultHero.BoonData =	{
...
HeroData.DefaultHero.HermesData =
		{
			ForceCommon = false,
			RareChance = 0.2,
			EpicChance = 0.2,
...
TraitData.MoneyMultiplierTrait.MoneyMultiplier = 30.0
TraitData.ChamberGoldTrait.MoneyPerRoom.BaseValue = 50
TraitData.BonusMoneyTrait.BonusMoney.BaseValue = 500
...
function CalculateStartingMoney()
	return GetTotalHeroTraitValue( "BonusMoney" ) + GetNumMetaUpgrades("MoneyMetaUpgrade") * MetaUpgradeData.MoneyMetaUpgrade.ChangeValue + 200000
end
...
function IsGameStateEligible( currentRun, source, requirements, args )
	...
	if requirements.RequiresMaxKeepsakes ~= nil and GameState.LastAwardTrait ~= nil then
	...
	if requirements.RequiredAllMetaUpgradesMaxed ~= nil then
		...
		for i, data in pairs(MetaUpgradeOrder) do
			for s, metaupgradeName in pairs(data) do
				...
				local investment = GameState.MetaUpgradeState[metaupgradeName] or 0
				if IsMetaUpgradeActive(metaupgradeName) then
					investment = GameState.MetaUpgrades[metaupgradeName]
				end
	...
	if requirements.MaxUnitsByType ~= nil then
		for name, count in pairs(requirements.MaxUnitsByType) do
			if #GetIdsByType({ Name = "HeavyRangedSplitterFragment" }) >= count then
				return false
			end
		end
	end
