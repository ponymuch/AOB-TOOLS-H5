...
function PickupWeaponKit( weaponKit )
	...
	if CurrentRun.NeedWeaponPickupBinkLoad then
...
function PlayInteractAnimation( interactableObjectId )
...
OnUsed{ "FishingPoint",
	function( triggerArgs )
		if CheckCooldown( "UsedFishingPoint", 0.75 ) then
			...
			if IsCombatEncounterActive( CurrentRun ) and canFishInEncounter then
				thread( InCombatText, CurrentRun.Hero.ObjectId, "UseBlockedByEnemies" )
				thread( PlayVoiceLines, HeroVoiceLines.InteractionBlockedVoiceLines, true )
				PlaySound({ Name = "/Leftovers/SFX/OutOfAmmo", Id = triggerArgs.triggeredById })
			elseif CurrentRun.Hero.OnLava then
				thread( InCombatText, CurrentRun.Hero.ObjectId, "UseBlockedByLava" )
				thread( PlayVoiceLines, HeroVoiceLines.InteractionBlockedVoiceLines, true )
				PlaySound({ Name = "/Leftovers/SFX/OutOfAmmo", Id = triggerArgs.triggeredById })
			else
...
function HandleThanatosEncounterReward( thanatos, args )
	...
	if (encounter.ThanatosKills or 0) > (encounter.PlayerKills or 0) then
		...
		CheckAchievement( { Name = "BigThanatosWin", CurrentValue = encounter.PlayerKills - encounter.ThanatosKills })
...
function PurchaseConsumableItem( currentRun, consumableItem, args )
	...
	if consumableItem.DropMoney ~= nil then
		thread( GushMoney, { Amount = consumableItem.DropMoney, LocationId = consumableItem.ObjectId, Source = consumableItem.Name, Delay = consumableItem.DropMoneyDelay } )
	end
...
function AttemptReroll( run, target )
	...
	run.NumRerolls = run.NumRerolls + 2
...
function AttemptRerollDoor( run, door )
	...
	local roomType = {"Boon", "WeaponUpgrade", "StackUpgrade", "HermesUpgrade"}
