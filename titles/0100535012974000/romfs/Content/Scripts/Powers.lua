...
function CheckOnDamagedPowers( victim, attacker, args )
	...
	if isCrit and attacker ~= nil and attacker.ObjectId == CurrentRun.Hero.ObjectId and victim ~= nil and victim.TriggersOnDamageEffects then
...
function CheckComboPowers( victim, attacker, triggerArgs, sourceWeaponData )
	...
	if attacker.ComboCount >= 1 and not attacker.ComboReady then
...
function ShieldFireClear( triggerArgs, args )
	if HeroHasTrait("ShieldLoadAmmoTrait") and CurrentRun.CurrentRoom.LoadedAmmo and CurrentRun.CurrentRoom.LoadedAmmo > 0 and triggerArgs.name == "ShieldWeaponRush" and CurrentRun.Hero.StoredAmmo and CurrentRun.CurrentRoom.LoadedAmmo then
		...
		if HeroHasTrait("ShieldLoadAmmo_ZeusRangedTrait" ) then
			for i = 1, numAmmo do
				thread(FireWeaponWithinRange, { Range = 350, SeekTarget = true, WeaponName = "ZeusShieldLoadAmmoStrike", InitialDelay = 0.1 * i, Delay = 0.25, Count = 1, BonusChance = GetTotalHeroTraitValue("BonusBolts") })
			end
		end
...
function UnloadAmmoThread( args )
	...
	for i = 1, count do
		...
		if victim ~= nil then
			FireWeaponFromUnit({ Weapon = "RangedWeapon", AutoEquip = true, Id = attacker.ObjectId, DestinationId = victim.ObjectId, FireFromTarget = true, Angle = ( angle + RandomFloat(-spread, spread)) * math.pi / 180 })
		elseif locationX and locationY then
			local dropLocation = SpawnObstacle({ Name = "InvisibleTarget", LocationX = locationX, LocationY = locationY })
			FireWeaponFromUnit({ Weapon = "RangedWeapon", AutoEquip = true, Id = attacker.ObjectId, DestinationId = dropLocation, FireFromTarget = true, Angle = ( angle + RandomFloat(-spread, spread))* math.pi / 180 })
			Destroy({Id = dropLocation })
		else
			FireWeaponFromUnit({ Weapon = "RangedWeapon", AutoEquip = true, Id = attacker.ObjectId, DestinationId = attacker.ObjectId, Angle = ( angle + RandomFloat(-spread, spread)) * math.pi / 180 })
		end
...
function CheckFistDetonation( victim, functionArgs, triggerArgs )
	if ( not victim.ActiveEffects or not victim.ActiveEffects.MarkRuptureTarget ) and triggerArgs.SourceWeapon == "FistWeaponSpecialDash" then
		local delay = 0.15
		CurrentRun.CurrentRoom.QueuedDetonations = CurrentRun.CurrentRoom.QueuedDetonations  or {}
		while CurrentRun.CurrentRoom.QueuedDetonations[_worldTime + delay ] and delay < 2 do
			delay = delay + 0.1
		end
		local key = _worldTime + delay
		CurrentRun.CurrentRoom.QueuedDetonations[_worldTime + delay] = victim
		wait( delay, RoomThreadName )
		...
		CurrentRun.CurrentRoom.QueuedDetonations[key] = nil
...
function CheckRecallTeleportToNearbyEnemy( weaponData, args )
		...
		if targetId ~= 0 and ActiveEnemies[targetId] ~= nil and not ActiveEnemies[targetId].IsDead then
			...
			DestroyOnDelay({ tempObstacle }, 0.05)
		end
